...
respon = re.get(https://www.law.go.kr/DRF/lawSearch.do?OC=nodam89&target=prec&type=XML)
print("---")
print(urlopen(url))
print("---")
print(response)
...
for i in trange(int(totalCnt / 20)):
    ...
    page += 1
    url = "https://www.law.go.kr/DRF/lawSearch.do?OC=nodam89&target=prec&type=XML&page={}".format(
        page
    )
    response = urlopen(url).read()
    xtree = ET.fromstring(response)
